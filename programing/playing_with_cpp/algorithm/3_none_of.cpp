...
bool ret1 = std::none_of(fooEven.begin(), fooEven.end(), [](int i){return i%2;}); 
 if(ret1==true)
    std::cout << "none of is odd \n";
  else
      std::cout << " some are odd\n";


bool ret2 = std::none_of(Vec1.begin(), Vec1.end(), [](int i){return i<0;}); 
 if(ret2==true)
    std::cout << "none of are negative \n";
  else
      std::cout << "some are negative  \n";


bool ret3 = std::none_of(Vec2.begin(), Vec2.end(), [](int i){return i<0;}); 
 if(ret3==true)
     std::cout << "none of are negative .\n";
  else
       std::cout << "some are negative \n";
...
bool ret4 = std::none_of(Vec1.begin(), Vec1.end(), AnyIsMinus<int>); 
 if(ret4==true)
...
bool ret5 = std::none_of(Vec2.begin(), Vec2.end(), AnyIsMinus<int>); 
 if(ret5==true)
    std::cout << "none are negative \n";
  else
      std::cout << " some are negative \n";
